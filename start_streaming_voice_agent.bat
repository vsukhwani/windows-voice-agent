@echo off
echo ========================================
echo    Starting Streaming Voice Agent
echo ========================================
echo.
echo This version uses streaming TTS for much
echo lower perceived latency in responses!
echo.
echo Server will be available at: http://localhost:7860
echo Client will be available at: http://localhost:3000
echo.
echo Press Ctrl+C in either window to stop
echo ========================================
echo.

cd /d "%~dp0"

REM Start server in background
echo Starting Server with streaming TTS...
start "Voice Agent Server" cmd /c "venv\Scripts\python.exe server\bot_windows_simple.py --host 0.0.0.0 --port 7860 & pause"

REM Wait for server to start
echo Waiting for server to initialize...
timeout /t 5 /nobreak >nul

REM Start client in background
echo Starting Client...
start "Voice Agent Client" cmd /c "cd client && npm run dev"

echo.
echo ========================================
echo Voice agent started with streaming TTS!
echo This provides much lower perceived latency.
echo.
echo Server: http://localhost:7860
echo Client: http://localhost:3000
echo ========================================
pause
